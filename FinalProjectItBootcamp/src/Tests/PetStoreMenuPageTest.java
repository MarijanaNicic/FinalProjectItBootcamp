...
public class PetStoreMenuPageTest {
	...
	@Test
	public void navValidLinks() throws InterruptedException {
		...
		List<WebElement> leftNavigationBar = 
				this.driver.findElements(By.xpath(this.locators.getProperty("leftNavigationBar")));
		for (int i = 0; i < leftNavigationBar.size(); i++) {
			int status = verifyURLStatus(leftNavigationBar.get(i).getAttribute("href"));
			AssertJUnit.assertTrue(status < 400);
		}
		
		List<WebElement> mainImageLinks = 
				this.driver.findElements(By.xpath(this.locators.getProperty("mainImage")));
		for (int i = 0; i < mainImageLinks.size(); i++) {
			int status = verifyURLStatus(mainImageLinks.get(i).getAttribute("href"));
			AssertJUnit.assertTrue(status < 400);
		}
		
		List<WebElement> quickLinksOnTop = 
				this.driver.findElements(By.xpath(this.locators.getProperty("quickLinks")));
		for (int i = 0; i < quickLinksOnTop.size(); i++) {
			int status = verifyURLStatus(quickLinksOnTop.get(i).getAttribute("href"));
			AssertJUnit.assertTrue(status < 400);
		}
		List<WebElement> MenuContent = 
				this.driver.findElements(By.xpath(this.locators.getProperty("MenuContent")));
		for (int i = 0; i < MenuContent.size(); i++) {
			int status = verifyURLStatus(MenuContent.get(i).getAttribute("href"));
			AssertJUnit.assertTrue(status < 400);
		}
	}
		...
		public void goToCorrectURL() throws InterruptedException {
			...
			driver.navigate().back();
			// testing left navigation bar links
		...
		// testing url links on top
			...
			//testing links in Menu Content
			
			correctURL.getCartLink(driver);
			correctURL.clickCartLink(driver);
			String currUrlCartLink = driver.getCurrentUrl();
			currUrlCartLink = currUrlCartLink.replaceAll(";jsessionid=[^?]*", "");
			Assert.assertEquals(currUrlCartLink,
					"https://petstore.octoperf.com/actions/Cart.action?viewCart=");
			
			driver.navigate().back();
			
			correctURL.getSingInLink(driver);
			correctURL.clickSingInLink(driver);
			String currUrlSingLink = driver.getCurrentUrl();
			currUrlSingLink = currUrlSingLink.replaceAll(";jsessionid=[^?]*", "");
			Assert.assertEquals(currUrlSingLink,
					"https://petstore.octoperf.com/actions/Account.action?signonForm=");
			
			driver.navigate().back();
			
			correctURL.getHelpLink(driver);
			correctURL.clickHelpLink(driver);
			String currUrlHelpLink = driver.getCurrentUrl();
			currUrlHelpLink = currUrlHelpLink.replaceAll(";jsessionid=[^?]*", "");
			Assert.assertEquals(currUrlHelpLink,
					"https://petstore.octoperf.com/actions/Account.action?signonForm=");
